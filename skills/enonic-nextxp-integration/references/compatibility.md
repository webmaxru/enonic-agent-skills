...
| Guillotine app | 8.x installed in XP | Latest from marketplace |
...
- Version 5.x requires React 19, Next.js 16, and Node.js 24.15.0+ as peer dependencies.
...
- Depends on `@enonic/react-components` (^7.0.0) for built-in view components and rich-text rendering.
...
### Scaffolding
- Use `npx degit enonic/starter-nextjs` to create a new project from the official starter.
- The starter includes boilerplate for routing, API routes, preview mode, SSG with ISR, and component registry.
- The starter imports `@enonic/nextjs-adapter/baseMappings` in `_mappings.ts` to register built-in component types.
