...
2. Confirm `queryDsl` / `queryDslConnection` are used instead of the removed `query` / `queryConnection` (removed in Guillotine 8, deprecated since 6 Update 1).
...
* If the deprecated `query` field is used (removed in Guillotine 8), read `references/compatibility.md` to migrate to `queryDsl` with DSL syntax.
